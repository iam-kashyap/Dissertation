...
Original file is located at
    https://colab.research.google.com/drive/13OTO_uatq2HhmSwPOPa1kwaE6bh-T7l5
...
# TODO: Change this path to your 2_Training directory
# replace this with the path to your project
# %cd /content/drive/MyDrive/Dissertation/2_Training
...
def get_data(pair, window, theta):
    ...
    train_df = pd.read_parquet(f'../1_DataTransformation/TransformedData/{theta}/{pair}/Window_{window}/train.parquet.gzip')
    val_df = pd.read_parquet(f'../1_DataTransformation/TransformedData/{theta}/{pair}/Window_{window}/validation.parquet.gzip')
    test_df = pd.read_parquet(f'../1_DataTransformation/TransformedData/{theta}/{pair}/Window_{window}/test.parquet.gzip')
...
with open('/content/drive/MyDrive/Dissertation/params.json', 'r') as f:
    params = json.load(f)
...
class DirectionalChangeEnv(gymnasium.Env):
    ...
    def step(self, action):
        ...
        # determine if the market is in consolidation period
        dccs = self.state[:-1,1]
        ends = self.state[1:,0]
        os_length = ends = dccs
...
class ModelTrainer(object):
    ...
    def train(self):
        self.model.learn(total_timesteps=50000)  # change timesteps to e.g. 10000 for a shorted training process to check it's working
...
pair = str('USDCAD')  # TODO: run for all pairs: AUDUSD, EURGBP, EURUSD and USDCAD
window = int(18)  # TODO: run for all windows (see e.g. /1_DataTransformation/TransformedData/0.00013/AUDUSD/ for list of windows)
theta = float(0.00023)  # TODO: run for all thresholds: 0.00013, 0.00017, 0.00023
